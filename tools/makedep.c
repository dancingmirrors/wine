...
static void add_native_artifact( struct makefile *make, const char *name )
{
    strarray_add_uniq( &make->clean_files, name );
}
...
static const char *strip_compiler_cache( const char *cmd )
{
    const char *base, *end;
    ...
    for (base = end; base > cmd; base--) if (base[-1] == '/') break;
    if (end - base < 6 || strncmp( end - 6, "ccache", 6 )) return cmd;
    while (*end == ' ') end++;
    return *end ? end : cmd;
}
...
static void output_source_y( struct makefile *make, struct incl_file *source, const char *obj )
{
    ...
    struct strarray bison_flags = get_expanded_make_var_array( make, "EXTRABISONFLAGS" );

    strarray_addall( &bison_flags, get_expanded_file_local_var( make, obj, "EXTRABISONFLAGS" ));

    if (find_include_file( make, header ))
    {
        output( "%s: %s\n", obj_dir_path( make, header ), source->filename );
        output( "\t%s%s", cmd_prefix( "BISON" ), bison );
        output_filenames( bison_flags );
        output( " -o %s.tab.$$$$.c --defines=$@ %s && rm -f %s.tab.$$$$.c\n",
                                    ...
                                    unsigned int arch )
{
    ...
    else if (archs.count > 1 && is_using_msvcrt( make ))
    {
        ...
        if (!so_dll_supported)
        {
            add_native_artifact( make, strmake( "%s.o", obj ));
            return;
        }
    }
    ...
    output( "\t%s%s -c -o $@ %s", cmd_prefix( is_cxx ? "CXX" : "CC" ),
            is_cxx ? var_cxx : var_cc, source->filename );
    output_filenames( defines );
    ...
    if ((source->file->flags & FLAG_ARM64EC_X64) && !strcmp( archs.str[arch], "arm64ec" ))
    {
        ...
        cmd->cmd = strip_compiler_cache( get_expanded_make_variable( make, "x86_64_CC" ));
        if (cflags) strarray_add( &cmd->args, cflags );
    }
    else
    {
        char *cflags = get_expanded_arch_var( make, "CFLAGS", arch );
        cmd->cmd = strip_compiler_cache( get_expanded_arch_var( make, "CC", arch ));
        if (cflags) strarray_add( &cmd->args, cflags );
    }
...
static void output_unix_lib( struct makefile *make )
{
    ...
    unsigned int i, arch = 0;  /* unix libs are always native */
    ...
    output_filename( output_makefile_name );
    output( "\n" );
    output( "\t%s%s -o $@", cmd_prefix( "CCLD" ), make->has_cxx ? "$(CXX)" : "$(CC)" );
    output_filenames( get_expanded_make_var_array( make, "UNIXLDFLAGS" ));
    for (i = 0; i < unix_deps.count; i++)
    {
        const char *start, *end = strrchr( unix_deps.str[i], '/' );

        if (!end) continue;
        for (start = end; start > unix_deps.str[i] && start[-1] != '/'; start--) {}
        output_filename( strmake( "'-Wl,-rpath,$$ORIGIN/../%.*s'", (int)(end - start), start ));
    }
    output_filenames_obj_dir( make, make->unixobj_files );
...
static void output_sources( struct makefile *make )
{
    ...
    if (make->staticlib)
    {
        for (arch = 0; arch < archs.count; arch++)
            ...
            else if (!arch && !make->extlib)
                add_native_artifact( make, make->staticlib );
    }
    else if (make->module)
    {
        for (arch = 0; arch < archs.count; arch++)
        {
            ...
            if (!make->importlib) continue;
            if (is_multiarch( arch ) || !is_native_arch_disabled( make ))
                output_import_lib( make, arch );
            else if (!arch)
                add_native_artifact( make, strmake( "lib%s.a", make->importlib ));
        }
...
{
    struct strarray ignores = empty_strarray;
    unsigned int i, arch;

    /* Architecture build artifacts. */
    for (i = 0; i < files.count; i++)
    {
        const char *file = files.str[i];
        ...
        for (arch = 1; arch < archs.count; arch++)
        {
            if (arch_dirs[arch] && !strncmp( file, arch_dirs[arch], strlen( arch_dirs[arch] )))
            {
                file = arch_dirs[arch];
                break;
            }
        }
        strarray_add_uniq( &ignores, file );
    }

    output_file = create_temp_file( dest );

    output( "# Automatically generated by make depend; DO NOT EDIT!!\n" );
    for (i = 0; i < ignores.count; i++)
    {
        if (!strchr( ignores.str[i], '/' ) || ignores.str[i][strlen( ignores.str[i] ) - 1] == '/')
            output( "/" );
        output( "%s\n", ignores.str[i] );
    }
