...
AC_DEFUN([WINE_CHECK_CCACHE],
[AC_ARG_WITH(ccache,AS_HELP_STRING([--without-ccache],[do not use ccache even if it is installed]))
AC_ARG_VAR(CCACHE,[compiler cache to prefix compiler commands with, or "no" to disable it])
...
case "x$CCACHE" in
  xno|xNO|xNo|xfalse|x0) CCACHE=no ;;
esac
AS_VAR_IF([CCACHE],[no],
  ...
  [AC_CHECK_PROGS(CCACHE,ccache,no)
   AS_VAR_IF([CCACHE],[no],[],
     [AC_CACHE_CHECK([whether ccache works],[wine_cv_ccache_works],
        [AS_IF([$CCACHE --version >/dev/null 2>&1],
               [wine_cv_ccache_works=yes],[wine_cv_ccache_works=no])])
      AS_VAR_IF([wine_cv_ccache_works],[yes],[],
        [WINE_WARNING([$CCACHE cannot be executed. Building without a compiler cache.])
         CCACHE=no])])])
AS_VAR_IF([CCACHE],[no],[],
  [AC_MSG_CHECKING([for the ccache cache directory])
   if test -z "$CCACHE_DIR"
   then
       CCACHE_DIR=`$CCACHE -p 2>/dev/null | sed -n 's/^.*cache_dir = //p' | sed 1q`
       if test -z "$CCACHE_DIR"
       then
           if test -d "$HOME/.ccache"
           then CCACHE_DIR=$HOME/.ccache
           else CCACHE_DIR=${XDG_CACHE_HOME:-$HOME/.cache}/ccache
           fi
       fi
   fi
   AC_MSG_RESULT([$CCACHE_DIR])
   test -n "$CCACHE_BASEDIR" || CCACHE_BASEDIR=`(cd "$srcdir" && pwd)`
   test -n "$CCACHE_BASEDIR" || CCACHE_BASEDIR=$ac_pwd
   wine_ccache_hashdir=""
   if test -n "$CCACHE_HASHDIR"
   then
       wine_ccache_hashdir="
export CCACHE_HASHDIR = $CCACHE_HASHDIR"
       export CCACHE_HASHDIR
   elif test -n "$CCACHE_NOHASHDIR"
   then
       wine_ccache_hashdir="
...
AC_DEFUN([WINE_CCACHE_WRAP],
[AS_VAR_IF([CCACHE],[no],[],
  [AS_VAR_COPY([wine_ccache_cmd],[$1])
   wine_ccache_prog=${wine_ccache_cmd%% *}
   case "${wine_ccache_prog##*/}" in
     ""|false|*ccache|ccache-*) ;;
     *)
       AC_MSG_CHECKING([whether to use ccache for $1])
       AS_IF([$CCACHE $wine_ccache_cmd --version >/dev/null 2>&1],
             [AS_VAR_SET([$1],["$CCACHE $wine_ccache_cmd"])
              AC_MSG_RESULT([yes])],
             [AC_MSG_RESULT([no])]) ;;
   esac])])
