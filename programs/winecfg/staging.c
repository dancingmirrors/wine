...
{
    WCHAR *value = get_reg_key(config_key, keypath(L"DXVA2"), L"backend", L"va");
    BOOL ret = wcsicmp(value, L"none") != 0;
    free(value);
    return ret;
}
static void vaapi_set(BOOL status)
{
    set_reg_key(config_key, keypath(L"DXVA2"), L"backend", status ? L"va" : L"none");
}
