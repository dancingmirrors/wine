...
{
    DEBUG_EVENT		de;
    DWORD               interrupt_deadline = 0;

    while (dbg_num_processes())
    {
        if (WaitForDebugEvent(&de, 200))
        {
            if (dbg_handle_debug_event(&de))
            {
                dbg_interrupt_requested = FALSE;
                break;
            }
        }
        else if (GetLastError() != ERROR_SEM_TIMEOUT) break;

        if (dbg_interrupt_requested)
        {
            if (!interrupt_deadline)
                interrupt_deadline = GetTickCount() + 2000;
            else if ((int)(GetTickCount() - interrupt_deadline) >= 0)
            {
                dbg_interrupt_requested = FALSE;
                dbg_printf("\nFailed to interrupt debuggee; it is still running.\n"
                           "Use 'kill' to terminate it or 'quit' to leave it running.\n");
                break;
            }
        }
        else interrupt_deadline = 0;
    }
    dbg_interrupt_requested = FALSE;
    dbg_interactiveP = TRUE;
}
...
enum dbg_start dbg_active_auto(int argc, char* argv[])
{
    ...
    dbg_use_wine_output = TRUE;
    dbg_houtput = INVALID_HANDLE_VALUE;
