...
{
    INTERRUPT_NONE,   /* nothing pending on the gdb socket */
    INTERRUPT_BREAK,  /* gdb asked us to break into the debuggee ('\003') */
    INTERRUPT_GONE,   /* gdb closed the connection */
};

static enum interrupt_status check_for_interrupt(struct gdb_context* gdbctx)
{
    char pkt;
    ...
    if (select( 0, &read_fds, NULL, &err_fds, &tv ) <= 0)
        return INTERRUPT_NONE;

    if (FD_ISSET( gdbctx->sock, &err_fds ))
        return INTERRUPT_GONE;

    ret = recv(gdbctx->sock, &pkt, 1, 0);
    if (ret == 0)
        return INTERRUPT_GONE;
    if (ret < 0) {
        ERR("read failed\n");
...
static void    wait_for_debuggee(struct gdb_context* gdbctx)
{
    ...
    for (;;)
    {
        if (!WaitForDebugEvent(&gdbctx->de, 10))
        {
            ...
            switch (check_for_interrupt(gdbctx))
            {
            case INTERRUPT_BREAK:
                ...
                while (!WaitForDebugEvent(&gdbctx->de, 100))
                {
                    if (GetLastError() != ERROR_SEM_TIMEOUT)
                        return;
                    if (check_for_interrupt(gdbctx) == INTERRUPT_GONE)
                        return;
                }
                break;
            case INTERRUPT_GONE:
...
static void packet_query_monitor_mem(struct gdb_context* gdbctx, int len, const char* str)
{
    ...
    while (VirtualQueryEx(gdbctx->process->handle, addr, &mbi, sizeof(mbi)) >= sizeof(mbi))
    {
        ...
        if ((DWORD_PTR)addr + mbi.RegionSize < (DWORD_PTR)addr) /* wrap around? */
            break;
        addr += mbi.RegionSize;
    }
    packet_reply(gdbctx, "OK");
