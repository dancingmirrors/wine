...
static inline BOOL buffer_has_space(const unsigned char *buffer, ULONG size, const unsigned char *end)
{
    return buffer <= end && size <= (ULONG_PTR)(end - buffer);
}

static inline void safe_buffer_increment(MIDL_STUB_MESSAGE *pStubMsg, ULONG size)
{
    if (!buffer_has_space(pStubMsg->Buffer, size,
                          (unsigned char *)pStubMsg->RpcMsg->Buffer + pStubMsg->BufferLength))
        RpcRaiseException(RPC_X_BAD_STUB_DATA);
...
{
    if (!buffer_has_space(pStubMsg->Buffer, size, pStubMsg->BufferEnd))
    {
...
static void validate_string_data(MIDL_STUB_MESSAGE *pStubMsg, ULONG bufsize, ULONG esize)
{
    ...
    if (!buffer_has_space(pStubMsg->Buffer, bufsize, pStubMsg->BufferEnd))
    {
