                 ...
                 HANDLE hproc, const struct location* frame)
{
    ...
    while (ctx->data < ctx->end_data)
    {
        ...
        case DW_OP_shra:
            if (stack[stk] >= sizeof(stack[stk]) * 8)
            {
                WARN("Ignoring location expression with out-of-range shift\n");
                return loc_err_too_complex;
            }
            stack[stk-1] = stack[stk-1] / ((DWORD_PTR)1 << stack[stk]); stk--; break;
        case DW_OP_div:
            if (!stack[stk])
            {
                /* See the "one breg" FIXME. */
                WARN("Ignoring location expression with division by zero\n");
                return loc_err_too_complex;
            }
        ...
        case DW_OP_mod:
            if (!stack[stk])
            {
                WARN("Ignoring location expression with modulo by zero\n");
                return loc_err_too_complex;
            }
            stack[stk-1] = stack[stk-1] % stack[stk]; stk--; break;
                                      ...
                                      ULONG_PTR offset)
{
    ...
    if (!line_range)
    {
        /* Avoid division by zero with a malformed .debug_line. */
        WARN("Invalid line number program (line_range == 0), skipping.\n");
        return FALSE;
    }
...
static inline BOOL dwarf2_div_faults( DWORD64 a, DWORD64 b )
{
    return b == 0 || ((INT64)b == -1 && a == ((DWORD64)1 << 63));
}
                                 ...
                                 const unsigned char* zp, union ctx *context)
{
    ...
    while (ctx.data < ctx.end_data)
    {
        ...
        case DW_OP_shra:
            if (stack[sp] >= sizeof(stack[sp]) * 8) stack[sp-1] = 0;
            else stack[sp-1] = (LONG_PTR)stack[sp-1] / (LONG_PTR)((DWORD64)1 << stack[sp]);
            sp--; break;
        case DW_OP_div:
            if (dwarf2_div_faults( stack[sp-1], stack[sp] )) stack[sp-1] = 0;
