...
static int init_info(ADL_CONTEXT_HANDLE ctx)
{
    ...
    /* Get GPUs. Proton enumerates the display adapter device interfaces through
     * cfgmgr32 here, which Wine 10.0 does not implement yet, so build the initial
     * GPU list from dxgi instead. The adapter LUIDs are the ones win32u assigns,
     * so they match what QueryDisplayConfig() reports below. */
    for (i = 0; !IDXGIFactory1_EnumAdapters1(dxgi_factory, i, &dxgi_adapter); ++i)
    {
        HRESULT hr = IDXGIAdapter1_GetDesc1(dxgi_adapter, &adapter_desc);

        IDXGIAdapter1_Release(dxgi_adapter);
        if (FAILED(hr)) continue;

        ctx->gpus = realloc(ctx->gpus, (ctx->gpu_count + 1) * sizeof(*ctx->gpus));
        gpu = &ctx->gpus[ctx->gpu_count];
        memset(gpu, 0, sizeof(*gpu));
        gpu->luid = adapter_desc.AdapterLuid;
        ...
        /* same layout as the device interface symlink win32u registers */
        snprintf(gpu->device_path, sizeof(gpu->device_path),
                 "\\\\?\\PCI#VEN_%04X&DEV_%04X&SUBSYS_%08X&REV_%02X#%08X#{5b45201d-f2f2-4f3b-85bb-30ff1f953599}",
                 adapter_desc.VendorId, adapter_desc.DeviceId, adapter_desc.SubSysId, adapter_desc.Revision, i);
        ++ctx->gpu_count;
    }
    ...
    for (i = 0; EnumDisplayDevicesA(NULL, i, &dd, EDD_GET_DEVICE_INTERFACE_NAME); ++i)
    {
        for (j = 0; j < ctx->adapter_count; ++j)
        {
            if (stricmp(ctx->adapters[j].gdi_device_name, dd.DeviceName)) continue;
            strcpy(ctx->adapters[j].gpu->device_string, dd.DeviceString);
            strcpy(ctx->adapters[j].driver_path, dd.DeviceKey);
        }
    }
