...
#define VA_CALL(func, params) WINE_UNIX_CALL(unix_##func, params)
...
struct video_decoder
{
    ...
    va_decoder_handle va_decoder;

    struct
    {
        void *data;
        UINT size;
    } buffers[DXVA2_FilmGrainBuffer + 1];
};
...
static BOOL dxva_decoder_is_supported_guid(REFGUID guid)
{
    return IsEqualGUID(guid, &DXVA2_ModeH264_E) || IsEqualGUID(guid, &DXVA2_ModeH264_F);
}

static struct get_decoder_caps_params decoder_caps;
static INIT_ONCE decoder_caps_once = INIT_ONCE_STATIC_INIT;

static BOOL CALLBACK init_decoder_caps(INIT_ONCE *once, void *param, void **context)
{
    if (VA_CALL(get_decoder_caps, &decoder_caps))
    {
        WARN("Failed to query decoder caps.\n");
        return FALSE;
    }

    TRACE("Supported sizes are %ux%u to %ux%u.\n", decoder_caps.min_width,
            decoder_caps.min_height, decoder_caps.max_width, decoder_caps.max_height);
    return TRUE;
}

static BOOL dxva_decoder_is_supported_size(unsigned int width, unsigned int height)
{
    if (!unix_available)
        return TRUE;

    if (!InitOnceExecuteOnce(&decoder_caps_once, init_decoder_caps, NULL, NULL))
        return TRUE;

    if (width < decoder_caps.min_width || width > decoder_caps.max_width
            || height < decoder_caps.min_height || height > decoder_caps.max_height)
    {
        WARN("Size %ux%u is outside of the supported range %ux%u to %ux%u.\n", width, height,
                decoder_caps.min_width, decoder_caps.min_height,
                decoder_caps.max_width, decoder_caps.max_height);
        return FALSE;
    }
...
static HRESULT WINAPI video_decoder_QueryInterface(IDirectXVideoDecoder *iface, REFIID riid, void **obj)
{
    ...
    if (IsEqualIID(riid, &IID_IDirectXVideoDecoder) ||
            IsEqualIID(riid, &IID_IUnknown))
    {
        *obj = iface;
...
static ULONG WINAPI video_decoder_Release(IDirectXVideoDecoder *iface)
{
    ...
    if (!refcount)
    {
        if (decoder->va_decoder)
        {
            struct destroy_decoder_params params = { .decoder = decoder->va_decoder };
            VA_CALL(destroy_decoder, &params);
        }
        for (i = 0; i < ARRAY_SIZE(decoder->buffers); ++i)
            free(decoder->buffers[i].data);
        if (decoder->render_target)
            IDirect3DSurface9_Release(decoder->render_target);
        for (i = 0; i < decoder->surface_count; ++i)
        {
            if (decoder->surfaces[i])
...
static HRESULT WINAPI video_decoder_GetCreationParameters(IDirectXVideoDecoder *iface,
        GUID *device_guid, DXVA2_VideoDesc *video_desc, DXVA2_ConfigPictureDecode *config,
        IDirect3DSurface9 ***surfaces, UINT *num_surfaces)
{
    struct video_decoder *decoder = impl_from_IDirectXVideoDecoder(iface);

    TRACE("%p, %p, %p, %p, %p, %p.\n", iface, device_guid, video_desc, config, surfaces, num_surfaces);

    if (device_guid)
        *device_guid = decoder->device;
    if (video_desc)
        *video_desc = decoder->video_desc;
    if (config)
        *config = decoder->config;
    if (surfaces)
        *surfaces = decoder->surfaces;
    if (num_surfaces)
        *num_surfaces = decoder->surface_count;

    return S_OK;
}

static UINT video_decoder_buffer_size(const struct video_decoder *decoder, UINT type)
{
    UINT frame_size = decoder->video_desc.SampleWidth * decoder->video_desc.SampleHeight;

    switch (type)
    {
        case DXVA2_BitStreamDateBufferType:
            return max(frame_size, 1u << 20);
        default:
            return 1u << 16;
    }
}

static HRESULT WINAPI video_decoder_GetBuffer(IDirectXVideoDecoder *iface, UINT type,
        void **buffer, UINT *size)
{
    ...
    if (type >= ARRAY_SIZE(decoder->buffers))
        return E_INVALIDARG;

    if (!decoder->buffers[type].data)
    {
        UINT alloc = video_decoder_buffer_size(decoder, type);
        if (!(decoder->buffers[type].data = calloc(1, alloc)))
            return E_OUTOFMEMORY;
        decoder->buffers[type].size = alloc;
    }
...
static HRESULT WINAPI video_decoder_BeginFrame(IDirectXVideoDecoder *iface,
        IDirect3DSurface9 *render_target, void *pvpv_data)
{
    struct video_decoder *decoder = impl_from_IDirectXVideoDecoder(iface);

    TRACE("%p, %p, %p.\n", iface, render_target, pvpv_data);

    if (decoder->render_target)
        IDirect3DSurface9_Release(decoder->render_target);
    if ((decoder->render_target = render_target))
        IDirect3DSurface9_AddRef(render_target);
...
static HRESULT WINAPI video_decoder_EndFrame(IDirectXVideoDecoder *iface, HANDLE *handle_complete)
{
    ...
    {
        IDirect3DSurface9_Release(decoder->render_target);
        decoder->render_target = NULL;
    }

    return S_OK;
}

static HRESULT WINAPI video_decoder_Execute(IDirectXVideoDecoder *iface,
        const DXVA2_DecodeExecuteParams *params)
{
    struct video_decoder *decoder = impl_from_IDirectXVideoDecoder(iface);
    const DXVA_PicParams_H264 *pic_params = NULL;
    const DXVA_Qmatrix_H264 *qmatrix = NULL;
    const DXVA_Slice_H264_Long *slices = NULL;
    DXVA_Slice_H264_Long *synth = NULL;
    const void *slice_control = NULL;
    UINT slice_control_size = 0, slice_count = 0, bitstream_size = 0;
    const void *bitstream = NULL;
    struct decode_h264_params decode;
    struct copy_surface_params copy;
    ...
    if (!decoder->va_decoder)
    {
        FIXME("No VA-API decode backend available.\n");
        return E_NOTIMPL;
    }

    for (i = 0; i < params->NumCompBuffers; ++i)
    {
        const DXVA2_DecodeBufferDesc *desc = &params->pCompressedBuffers[i];
        UINT type = desc->CompressedBufferType;
        const void *data;

        if (type >= ARRAY_SIZE(decoder->buffers) || !decoder->buffers[type].data)
            continue;
        data = (const char *)decoder->buffers[type].data + desc->DataOffset;
        ...
        switch (type)
        {
            ...
            case DXVA2_SliceControlBufferType:
                slice_control = data;
                slice_control_size = desc->DataSize;
                break;
            case DXVA2_BitStreamDateBufferType:
                bitstream = data;
                bitstream_size = desc->DataSize;
                break;
            default:
                break;
        }
    ...
    if (!pic_params || !slice_control || !slice_control_size || !bitstream)
    {
        WARN("Incomplete decode buffers (pic %p, slices %p/%u, bitstream %p).\n",
             pic_params, slice_control, slice_control_size, bitstream);
        return E_INVALIDARG;
    }

    if (decoder->config.ConfigBitstreamRaw == 1)
    {
        slices = slice_control;
        slice_count = slice_control_size / sizeof(DXVA_Slice_H264_Long);
        if (!slice_count)
            return E_INVALIDARG;
    }
    else
    {
        const DXVA_Slice_H264_Short *shorts = slice_control;

        slice_count = slice_control_size / sizeof(DXVA_Slice_H264_Short);
        if (!slice_count)
            return E_INVALIDARG;
        if (!(synth = calloc(slice_count, sizeof(*synth))))
            return E_OUTOFMEMORY;
        for (i = 0; i < slice_count; ++i)
        {
            if (!h264_parse_short_slice(pic_params, bitstream, bitstream_size, &shorts[i], &synth[i]))
            {
                WARN("Failed to parse short H.264 slice %u/%u.\n", i, slice_count);
                free(synth);
                return E_FAIL;
            }
        }
    ...
    decode.decoder = decoder->va_decoder;
    decode.pic_params = pic_params;
    decode.qmatrix = qmatrix;
    decode.slices = slices;
    decode.slice_count = slice_count;
    decode.bitstream = bitstream;
    decode.bitstream_size = bitstream_size;
    if (VA_CALL(decode_h264, &decode))
    {
        free(synth);
        WARN("VA-API decode failed.\n");
        return E_FAIL;
    }
    free(synth);
    ...
    if (FAILED(hr = IDirect3DSurface9_LockRect(decoder->render_target, &locked, NULL, 0)))
    {
        WARN("Failed to lock render target, hr %#lx.\n", hr);
        return hr;
    }

    copy.decoder = decoder->va_decoder;
    copy.surface_index = pic_params->CurrPic.Index7Bits;
    copy.dst = locked.pBits;
    copy.dst_pitch = locked.Pitch;
    copy.width = decoder->video_desc.SampleWidth;
    copy.height = decoder->video_desc.SampleHeight;
    if (VA_CALL(copy_surface, &copy))
        WARN("Failed to copy decoded surface to the render target.\n");

    IDirect3DSurface9_UnlockRect(decoder->render_target);
    return S_OK;
}
        ...
        IDirect3DSurface9 **surfaces, HANDLE *shared_handle)
{
    ...
    {
        if (FAILED(hr = IDirect3DDevice9_CreateOffscreenPlainSurface(manager->device, width, height, format,
                D3DPOOL_SYSTEMMEM, &surfaces[i], NULL)))
            break;
    }
        ...
        UINT *count, GUID **guids)
{
    ...
    if (!(*guids = CoTaskMemAlloc(2 * sizeof(**guids))))
        return E_OUTOFMEMORY;

    (*guids)[0] = DXVA2_ModeH264_E;
    (*guids)[1] = DXVA2_ModeH264_F;
    *count = 2;
        ...
        REFGUID guid, UINT *count, D3DFORMAT **formats)
{
    ...
    if (!dxva_decoder_is_supported_guid(guid))
    {
        FIXME("Unsupported decoder %s.\n", debugstr_guid(guid));
        return E_NOTIMPL;
    }

    if (!(*formats = CoTaskMemAlloc(sizeof(**formats))))
        return E_OUTOFMEMORY;

    (*formats)[0] = D3DFMT_NV12;
    *count = 1;
        ...
        REFGUID guid, const DXVA2_VideoDesc *video_desc, void *reserved, UINT *count, DXVA2_ConfigPictureDecode **configs)
{
    ...
    if (!video_desc || !dxva_decoder_is_supported_size(video_desc->SampleWidth, video_desc->SampleHeight))
        return E_INVALIDARG;

    if (!(config = CoTaskMemAlloc(sizeof(*config))))
        return E_OUTOFMEMORY;

    memset(config, 0, sizeof(*config));
    config->guidConfigBitstreamEncryption = DXVA2_NoEncrypt;
    config->guidConfigMBcontrolEncryption = DXVA2_NoEncrypt;
    config->guidConfigResidDiffEncryption = DXVA2_NoEncrypt;
        ...
        UINT num_surfaces, IDirectXVideoDecoder **decoder)
{
    ...
    TRACE("%p, %s, %p, %p, %p, %u, %p.\n", iface, debugstr_guid(guid),
            video_desc, config, rts, num_surfaces, decoder);
    ...
    *decoder = NULL;
    ...
    if (!dxva_decoder_is_supported_size(video_desc->SampleWidth, video_desc->SampleHeight))
        return E_INVALIDARG;

    if (!(object = calloc(1, sizeof(*object))))
        return E_OUTOFMEMORY;

    if (num_surfaces && !(object->surfaces = calloc(num_surfaces, sizeof(*object->surfaces))))
    {
        free(object);
        return E_OUTOFMEMORY;
    }
    ...
    {
        if ((object->surfaces[i] = rts[i]))
            IDirect3DSurface9_AddRef(rts[i]);
    }

    if (unix_available)
    {
        struct create_decoder_params params =
        {
            .width = video_desc->SampleWidth,
            .height = video_desc->SampleHeight,
            .surface_count = num_surfaces,
        };

        if (VA_CALL(create_decoder, &params))
        {
            WARN("Failed to create a VA-API decoder.\n");
            IDirectXVideoDecoder_Release(&object->IDirectXVideoDecoder_iface);
            return E_FAIL;
        }

        object->va_decoder = params.decoder;
    }
...
static BOOL va_backend_enabled(void)
{
    WCHAR buffer[16];
    DWORD size = sizeof(buffer), type;
    BOOL enabled = TRUE;
    HKEY key;

    if (!RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\Wine\\DXVA2", 0, KEY_READ, &key))
    {
        if (!RegQueryValueExW(key, L"backend", NULL, &type, (BYTE *)buffer, &size)
                && type == REG_SZ && !lstrcmpiW(buffer, L"none"))
            enabled = FALSE;
        RegCloseKey(key);
    }
...
BOOL WINAPI DllMain(HINSTANCE instance, DWORD reason, void *reserved)
{
    if (reason == DLL_PROCESS_ATTACH)
    {
        ...
        if (!va_backend_enabled())
            TRACE("VA-API DXVA2 backend disabled via the registry.\n");
        else if (!__wine_init_unix_call())
            unix_available = TRUE;
        else
            WARN("Failed to initialize the VA-API unix library.\n");
    }
