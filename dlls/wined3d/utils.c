...
{
    static const enum wined3d_format_id height_scaled_formats[] =
    {
        WINED3DFMT_YV12,
        WINED3DFMT_NV12,
    };
    ...
    for (i = 0; i < ARRAY_SIZE(height_scaled_formats); ++i)
    {
        if (!(format = get_format_internal(adapter, height_scaled_formats[i])))
            return FALSE;

        format->attrs |= WINED3D_FORMAT_ATTR_HEIGHT_SCALE;
...
unsigned int wined3d_format_get_plane_idx(const struct wined3d_adapter *adapter,
        const struct wined3d_format *format, const struct wined3d_format *view_format)
{
    unsigned int i;
    ...
    for (i = 0; i < ARRAY_SIZE(format->plane_formats); ++i)
    {
        const struct wined3d_format *plane_format = wined3d_get_format(adapter, format->plane_formats[i], 0);

        if (plane_format->typeless_id && plane_format->typeless_id == view_format->typeless_id)
            return i;
    }
