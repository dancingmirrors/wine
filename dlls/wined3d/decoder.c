...
struct wined3d_decoder
{
    ...
    struct wined3d_decoder_caps caps;
...
bool wined3d_decoder_is_supported_codec(struct wined3d_adapter *adapter, const GUID *codec)
{
...
void wined3d_decoder_get_caps(struct wined3d_adapter *adapter, const GUID *codec,
        enum wined3d_format_id output_format, struct wined3d_decoder_caps *caps)
{
    caps->min_width = 16;
    caps->min_height = 16;
    caps->max_width = 4096;
    ...
    if (adapter->decoder_ops->get_caps && !adapter->decoder_ops->get_caps(adapter, codec, output_format, caps))
        WARN("Failed to query decoder caps; assuming a maximum size of %ux%u.\n", caps->max_width, caps->max_height);

    TRACE("Codec %s supports sizes from %ux%u to %ux%u.\n", debugstr_guid(codec),
            caps->min_width, caps->min_height, caps->max_width, caps->max_height);
}

static bool is_supported_size(const struct wined3d_decoder_caps *caps, unsigned int width, unsigned int height)
{
    return width >= caps->min_width && width <= caps->max_width
            && height >= caps->min_height && height <= caps->max_height;
}
        ...
        struct wined3d_device *device, const struct wined3d_decoder_desc *desc)
{
    ...
    wined3d_decoder_get_caps(device->adapter, &desc->codec, desc->output_format, &decoder->caps);

    if (!is_supported_size(&decoder->caps, desc->width, desc->height))
    {
        WARN("Size %ux%u is outside of the supported range %ux%u to %ux%u; returning E_INVALIDARG.\n",
                desc->width, desc->height, decoder->caps.min_width, decoder->caps.min_height,
                decoder->caps.max_width, decoder->caps.max_height);
        return E_INVALIDARG;
    }
        ...
        const struct wined3d_decoder_desc *desc, struct wined3d_decoder **decoder)
{
    ...
    if (!wined3d_decoder_is_supported_codec(device->adapter, &desc->codec))
    {
    ...
    wined3d_mutex_lock();
    hr = device->adapter->decoder_ops->create(device, desc, decoder);
    wined3d_mutex_unlock();
    return hr;
}
...
static bool wined3d_decoder_vk_get_caps(struct wined3d_adapter *adapter, const GUID *codec,
        enum wined3d_format_id output_format, struct wined3d_decoder_caps *caps)
{
    VkVideoDecodeH264CapabilitiesKHR h264_caps = {.sType = VK_STRUCTURE_TYPE_VIDEO_DECODE_H264_CAPABILITIES_KHR};
    VkVideoDecodeCapabilitiesKHR decode_caps = {.sType = VK_STRUCTURE_TYPE_VIDEO_DECODE_CAPABILITIES_KHR};
    VkVideoProfileInfoKHR profile = {.sType = VK_STRUCTURE_TYPE_VIDEO_PROFILE_INFO_KHR};
    VkVideoCapabilitiesKHR vk_caps = {.sType = VK_STRUCTURE_TYPE_VIDEO_CAPABILITIES_KHR};
    ...
    if (output_format != WINED3DFMT_NV12_PLANAR)
    {
        WARN("Unhandled output format %s.\n", debug_d3dformat(output_format));
        return false;
    }

    if (!vk_info->supported[WINED3D_VK_KHR_VIDEO_QUEUE] || !vk_info->supported[WINED3D_VK_KHR_VIDEO_DECODE_H264])
        return false;

    fill_vk_profile_info(&profile, codec, output_format);

    vk_caps.pNext = &decode_caps;
    decode_caps.pNext = &h264_caps;
    if ((vr = VK_CALL(vkGetPhysicalDeviceVideoCapabilitiesKHR(
            adapter_vk->physical_device, &profile, &vk_caps))) != VK_SUCCESS)
    {
        WARN("Failed to query video capabilities, vr %s.\n", wined3d_debug_vkresult(vr));
        return false;
    }

    caps->min_width = vk_caps.minCodedExtent.width;
    caps->min_height = vk_caps.minCodedExtent.height;
        ...
        struct wined3d_context_vk *context_vk, struct wined3d_decoder_output_view *view)
{
    ...
    if (texture_vk->image.command_buffer_id == context_vk->current_command_buffer.id
            || decoder_vk->command_buffer_id > context_vk->completed_command_buffer_id)
    {
        ...
        struct wined3d_decoder_image_vk *image)
{
    ...
    {
        if (!is_supported_size(&decoder_vk->d.caps, width, height))
        {
            static unsigned int once;

            if (!once++)
                WARN("Stream resized to %ux%u, which the device does not support.\n", width, height);
            return false;
        }
        ...
        struct wined3d_decoder_output_view_vk *output_view_vk, unsigned int slot_index)
{
    ...
    const struct wined3d_vk_info *vk_info = context_vk->vk_info;
    VkImageSubresourceRange vk_range, src_range;
    ...
    vk_range.levelCount = 1;
    vk_range.baseArrayLayer = output_view_vk->v.desc.u.texture.layer_idx;
    vk_range.layerCount = 1;

    src_range.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    src_range.baseMipLevel = 0;
    src_range.levelCount = 1;
    src_range.baseArrayLayer = decoder_vk->layered_dpb ? slot_index : 0;
    src_range.layerCount = 1;

    wined3d_context_vk_image_barrier(context_vk, command_buffer,
            VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
            0, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_VIDEO_DECODE_DST_KHR,
            VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, src_image, &src_range);

    wined3d_context_vk_image_barrier(context_vk, command_buffer,
            VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
            vk_access_mask_from_bind_flags(texture_vk->t.resource.bind_flags), VK_ACCESS_TRANSFER_WRITE_BIT,
            texture_vk->layout, dst_layout, texture_vk->image.vk_image, &vk_range);
    ...
    wined3d_context_vk_image_barrier(context_vk, command_buffer,
            ...
            dst_layout, texture_vk->layout, texture_vk->image.vk_image, &vk_range);

    wined3d_context_vk_image_barrier(context_vk, command_buffer,
            VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
            VK_ACCESS_TRANSFER_READ_BIT, 0, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
            VK_IMAGE_LAYOUT_VIDEO_DECODE_DST_KHR, src_image, &src_range);
}
        ...
        const DXVA_PicParams_H264 *h264_params, const void *slice_control, unsigned int slice_control_size)
{
    ...
    if (!wined3d_decoder_vk_prepare_image(decoder_vk, context_vk,
    ...
    VK_CALL(vkCmdPipelineBarrier(decoder_vk->command_buffer.vk_command_buffer,
        ...
        unsigned int bitstream_size, unsigned int slice_control_size)
{
    ...
    const void *parameters, *slice_control, *bitstream;
    struct wined3d_bo_address bo_addr;
    struct wined3d_bo_vk decode_bo;
    unsigned int bitstream_range;
    void *map_ptr;

    wined3d_buffer_load_location(decoder_vk->d.bitstream, &context_vk->c, WINED3D_LOCATION_BUFFER);
    bitstream = wined3d_buffer_load_sysmem(decoder_vk->d.bitstream, &context_vk->c);
    ...
    bitstream_range = align(bitstream_size, decoder_vk->bitstream_alignment);
    if (!wined3d_context_vk_create_bo(context_vk,
        max(bitstream_range, WINED3D_ALLOCATOR_MIN_BLOCK_SIZE / 2 + 1),
        VK_BUFFER_USAGE_VIDEO_DECODE_SRC_BIT_KHR, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &decode_bo))
    {
        ERR("Failed to create bitstream buffer.\n");
        submit_decode_command_buffer(decoder_vk, context_vk);
        return;
    }
    bo_addr.buffer_object = &decode_bo.b;
    bo_addr.addr = NULL;
    if (!(map_ptr = wined3d_context_map_bo_address(context, &bo_addr, bitstream_size, WINED3D_MAP_WRITE)))
    {
        ERR("Failed to map bitstream buffer.\n");
        wined3d_context_vk_destroy_bo(context_vk, &decode_bo);
        submit_decode_command_buffer(decoder_vk, context_vk);
        return;
    }
    memcpy(map_ptr, bitstream, bitstream_size);
    wined3d_context_unmap_bo_address(context, &bo_addr, 0, NULL);

    decode_info.srcBuffer = decode_bo.vk_buffer;
    decode_info.srcBufferOffset = decode_bo.b.buffer_offset;
    decode_info.srcBufferRange = bitstream_range;
    ...
    wined3d_context_vk_reference_bo(context_vk, &decode_bo);
    wined3d_context_vk_destroy_bo(context_vk, &decode_bo);
...
const struct wined3d_decoder_ops wined3d_decoder_vk_ops =
{
    ...
    .get_caps = wined3d_decoder_vk_get_caps,
