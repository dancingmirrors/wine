        ...
        struct wined3d_resource *resource, BOOL mip_slice, BOOL allow_srgb_toggle)
{
    ...
    else
    {
        ...
        {
            if (wined3d_format_get_plane_idx(adapter, resource->format, format) == ~0u)
            {
                WARN("Trying to create view with format %s, which is not a plane of format %s.\n",
        ...
        VkImageUsageFlags usage)
{
    ...
    else if ((plane_idx = wined3d_format_get_plane_idx(device_vk->d.adapter,
            resource->format, &format_vk->f)) != ~0u)
    {
        create_info.subresourceRange.aspectMask = plane_idx ? VK_IMAGE_ASPECT_PLANE_1_BIT
