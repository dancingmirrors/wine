...
HRESULT CDECL wined3d_device_get_video_decode_caps(struct wined3d_device *device, const GUID *codec,
        enum wined3d_format_id output_format, struct wined3d_decoder_caps *caps)
{
    TRACE("device %p, codec %s, output_format %s, caps %p.\n",
            device, debugstr_guid(codec), debug_d3dformat(output_format), caps);

    if (!wined3d_decoder_is_supported_codec(device->adapter, codec))
    {
        WARN("Codec %s is not supported; returning E_INVALIDARG.\n", debugstr_guid(codec));
        return E_INVALIDARG;
    }

    wined3d_decoder_get_caps(device->adapter, codec, output_format, caps);
    return S_OK;
}
