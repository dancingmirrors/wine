...
static void async_reader_deliver_sample(struct async_reader *reader, struct sample *sample)
{
    ...
    read_compressed = reader->streams[sample->stream - 1].read_compressed;
...
static DWORD WINAPI stream_read_thread(void *arg)
{
    ...
    while (reader->running && stream->running)
    {
        ...
        if (!(sample = calloc(1, sizeof(*sample))))
        {
            ...
            stream->read_result = E_OUTOFMEMORY;
            WakeConditionVariable(&reader->callback_cv);
            continue;
        }
...
static HRESULT async_reader_open(struct async_reader *reader, IWMReaderCallback *callback, void *context)
{
    ...
    for (index = 0; index < reader->stream_count; ++index)
        reader->streams[index].selection = WMT_ON;
        ...
        WORD count, WORD *stream_numbers, WMT_STREAM_SELECTION *selections)
{
    ...
    EnterCriticalSection(&reader->callback_cs);
    for (i = 0; i < count && reader->streams; ++i)
    {
        if (stream_numbers[i] && stream_numbers[i] <= reader->stream_count)
            reader->streams[stream_numbers[i] - 1].selection = selections[i];
    }
    LeaveCriticalSection(&reader->callback_cs);
        ...
        WORD stream_number, BOOL compressed)
{
    ...
    if (FAILED(hr = IWMSyncReader2_SetReadStreamSamples(reader->reader, stream_number, compressed)))
        return hr;

    EnterCriticalSection(&reader->callback_cs);
    if (reader->streams && stream_number && stream_number <= reader->stream_count)
        reader->streams[stream_number - 1].read_compressed = !!compressed;
    LeaveCriticalSection(&reader->callback_cs);
        ...
        const WCHAR *name, WMT_ATTR_DATATYPE type, const BYTE *value, WORD length)
{
    ...
    if (output_num >= reader->stream_count)
    {
        LeaveCriticalSection(&reader->cs);
        return E_INVALIDARG;
    }
    ...
    {
        if (type != WMT_TYPE_BOOL || !value || length < sizeof(BOOL))
            hr = E_INVALIDARG;
        else
        {
            stream->dedicated_delivery_thread = *(const BOOL *)value;
            hr = S_OK;
        }
    }
    else
