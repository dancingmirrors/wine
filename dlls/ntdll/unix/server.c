...
/***********************************************************************
 *           wine_server_alloc_object_attributes
 *
 * Exported version of alloc_object_attributes() for the other Unix libraries.
 */
unsigned int wine_server_alloc_object_attributes( const OBJECT_ATTRIBUTES *attr, struct object_attributes **ret,
                                                  data_size_t *ret_len )
{
    return alloc_object_attributes( attr, ret, ret_len );
}
...
static int setup_config_dir(void)
{
    ...
    if (chdir( config_dir ) == -1)
    {
        ...
        if ((p = (char *)strrchr( config_dir, '/' )) && p != config_dir)
        {
