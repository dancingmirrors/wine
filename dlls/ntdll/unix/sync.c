...
#define NTSYNC_STATS_PERIOD 20000
static int ntsync_stats_enabled = -1;
static LONG ntsync_stat_fast;
static LONG ntsync_stat_fallback;
static LONG ntsync_stat_signalwait_fast;
static LONG ntsync_stat_signalwait_fallback;

static BOOL ntsync_stats_on(void)
{
    if (ntsync_stats_enabled < 0)
    {
        const char *e = getenv( "WINE_NTSYNC_STATS" );
        ntsync_stats_enabled = (e && e[0] && e[0] != '0') ? 1 : 0;
    }
    return ntsync_stats_enabled;
}

static void ntsync_stats_tick(void)
{
    LONG total = ntsync_stat_fast + ntsync_stat_fallback;
    if (total % NTSYNC_STATS_PERIOD) return;
    fprintf( stderr,
             "ntsync: waits fast=%d fallback=%d | signal_and_wait fast=%d fallback=%d\n",
             (int)ntsync_stat_fast, (int)ntsync_stat_fallback,
             (int)ntsync_stat_signalwait_fast, (int)ntsync_stat_signalwait_fallback );
}
                             ...
                             BOOLEAN alertable, const LARGE_INTEGER *timeout )
{
    ...
    for (int i = 0; i < count; ++i)
    {
        if ((ret = get_inproc_sync( handles[i], INPROC_SYNC_UNKNOWN, SYNCHRONIZE, &stack[i], &syncs[i] )))
        {
            ...
            if (ret == STATUS_NOT_IMPLEMENTED && ntsync_stats_on())
            {
                InterlockedIncrement( &ntsync_stat_fallback );
                ntsync_stats_tick();
            }
            return ret;
        }
    ...
    if (ntsync_stats_on())
    {
        InterlockedIncrement( &ntsync_stat_fast );
        ntsync_stats_tick();
    }
    return ret;
}
                                        ...
                                        BOOLEAN alertable, const LARGE_INTEGER *timeout )
{
    ...
    if ((ret = get_inproc_sync( signal, INPROC_SYNC_UNKNOWN, 0, &stack_signal, &signal_sync )))
    {
        if (ret == STATUS_NOT_IMPLEMENTED && ntsync_stats_on())
            InterlockedIncrement( &ntsync_stat_signalwait_fallback );
        return ret;
    }
    if ((ret = check_signal_access( signal_sync ))) goto done;

    if ((ret = get_inproc_sync( wait, INPROC_SYNC_UNKNOWN, SYNCHRONIZE, &stack_wait, &wait_sync )))
    {
        if (ret == STATUS_NOT_IMPLEMENTED && ntsync_stats_on())
            InterlockedIncrement( &ntsync_stat_signalwait_fallback );
        goto done;
    }
    if (ntsync_stats_on()) InterlockedIncrement( &ntsync_stat_signalwait_fast );
