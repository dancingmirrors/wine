...
static LONGLONG get_qpc_frequency(void)
{
    ...
    if (!frequency)
    {
        ...
        frequency = performance_frequency.QuadPart;
    }

    return frequency;
}

static void dxgi_frame_stats_update(struct dxgi_frame_stats *frame_stats)
{
    LONGLONG qpc_frequency = get_qpc_frequency(), elapsed, vblanks;
    LARGE_INTEGER qpc;

    QueryPerformanceCounter(&qpc);

    if (!frame_stats->refresh_rate
            || qpc.QuadPart - frame_stats->refresh_rate_qpc >= qpc_frequency)
    {
        frame_stats->refresh_rate = get_display_frequency();
        frame_stats->refresh_rate_qpc = qpc.QuadPart;
    }

    elapsed = qpc.QuadPart - frame_stats->vblank_qpc;
    if (!frame_stats->vblank_qpc || elapsed < 0)
    {
        frame_stats->vblank_qpc = qpc.QuadPart;
        return;
    }

    vblanks = elapsed * frame_stats->refresh_rate / qpc_frequency;
    frame_stats->sync_count += vblanks;
    frame_stats->vblank_qpc += vblanks * qpc_frequency / frame_stats->refresh_rate;
}

static void dxgi_frame_stats_invalidate(struct dxgi_frame_stats *frame_stats)
{
    frame_stats->vblank_qpc = 0;
}

static HRESULT dxgi_frame_stats_get(struct dxgi_frame_stats *frame_stats,
        unsigned int present_count, DXGI_FRAME_STATISTICS *stats)
{
    BOOL disjoint = !frame_stats->vblank_qpc;

    dxgi_frame_stats_update(frame_stats);

    stats->PresentCount = present_count;
    stats->PresentRefreshCount = frame_stats->present_count;
    stats->SyncRefreshCount = frame_stats->sync_count;
    stats->SyncQPCTime.QuadPart = frame_stats->vblank_qpc;
    stats->SyncGPUTime.QuadPart = 0;

    return disjoint ? DXGI_ERROR_FRAME_STATISTICS_DISJOINT : S_OK;
}
        ...
        unsigned int sync_interval, unsigned int flags)
{
    ...
    if (SUCCEEDED(hr = wined3d_swapchain_present(swapchain->wined3d_swapchain, NULL, NULL, NULL, sync_interval, 0)))
    {
        ...
        wined3d_mutex_lock();
        dxgi_frame_stats_update(&swapchain->frame_stats);
        swapchain->frame_stats.present_count = swapchain->frame_stats.sync_count;
        wined3d_mutex_unlock();
    }
    return hr;
}
        ...
        BOOL fullscreen, IDXGIOutput *target)
{
    ...
    dxgi_frame_stats_invalidate(&swapchain->frame_stats);
        ...
        const DXGI_MODE_DESC *target_mode_desc)
{
    ...
    if (SUCCEEDED(hr = dxgi_swapchain_resize_target(state, target_mode_desc)))
    {
        wined3d_mutex_lock();
        dxgi_frame_stats_invalidate(&swapchain->frame_stats);
        wined3d_mutex_unlock();
        ...
        DXGI_FRAME_STATISTICS *stats)
{
    ...
    wined3d_mutex_lock();
    hr = dxgi_frame_stats_get(&swapchain->frame_stats, swapchain->present_count, stats);
    wined3d_mutex_unlock();
...
{
    FIXME("iface %p, type %#x, size %#x, metadata %p semi-stub!\n", iface, type, size, metadata);
...
struct d3d12_swapchain
{
    ...
    struct dxgi_frame_stats frame_stats;
};
        ...
        const DXGI_MODE_DESC *target_mode_desc)
{
    ...
    if (SUCCEEDED(hr = dxgi_swapchain_resize_target(swapchain->state, target_mode_desc)))
    {
        dxgi_frame_stats_invalidate(&swapchain->frame_stats);
    }
        ...
        DXGI_FRAME_STATISTICS *stats)
{
    ...
    return dxgi_frame_stats_get(&swapchain->frame_stats, swapchain->frame_number, stats);
}
