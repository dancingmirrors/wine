        ...
        UINT *format_support)
{
    ...
    *format_support = 0;
    ...
    if (*format_support & (D3D11_FORMAT_SUPPORT_BUFFER | D3D11_FORMAT_SUPPORT_TEXTURE1D
            | D3D11_FORMAT_SUPPORT_TEXTURE2D | D3D11_FORMAT_SUPPORT_TEXTURE3D))
        ...
        void *feature_support_data, UINT feature_support_data_size)
{
    ...
    switch (feature)
    {
        ...
        case D3D11_FEATURE_FORMAT_SUPPORT2:
        {
            ...
            if (FAILED(hr = d3d11_device_CheckFormatSupport(iface, data->InFormat, &format_support)))
            {
            ...
            data->OutFormatSupport2 = 0;
            if (format_support & D3D11_FORMAT_SUPPORT_TYPED_UNORDERED_ACCESS_VIEW)
                data->OutFormatSupport2 |= D3D11_FORMAT_SUPPORT2_UAV_TYPED_STORE;

            switch (data->InFormat)
            {
                case DXGI_FORMAT_R32_UINT:
                case DXGI_FORMAT_R32_SINT:
                    data->OutFormatSupport2 |= D3D11_FORMAT_SUPPORT2_UAV_ATOMIC_ADD
                ...
                case DXGI_FORMAT_R32_FLOAT:
                    if (format_support & D3D11_FORMAT_SUPPORT_TYPED_UNORDERED_ACCESS_VIEW)
                        data->OutFormatSupport2 |= D3D11_FORMAT_SUPPORT2_UAV_TYPED_LOAD;
                    break;
        ...
        ID3D11VideoDevice1 *iface, const GUID *profile, DXGI_FORMAT format, BOOL *supported)
{
    ...
    *supported = (format == DXGI_FORMAT_NV12);

    return S_OK;
}

static bool d3d11_video_device_check_decoder(struct d3d_device *device,
        const GUID *profile, DXGI_FORMAT format, unsigned int width, unsigned int height)
{
    ...
    {
        WARN("Unsupported output format %#x.\n", format);
        return false;
    }

    if (FAILED(hr = wined3d_device_get_video_decode_caps(device->wined3d_device,
            profile, wined3dformat_from_dxgi_format(format), &caps)))
    {
        WARN("Failed to get decoder caps for profile %s, hr %#lx.\n", debugstr_guid(profile), hr);
        return false;
    }

    if (width < caps.min_width || width > caps.max_width
            || height < caps.min_height || height > caps.max_height)
    {
        WARN("Size %ux%u is outside of the supported range %ux%u to %ux%u.\n",
                width, height, caps.min_width, caps.min_height, caps.max_width, caps.max_height);
        return false;
    }
        ...
        const D3D11_VIDEO_DECODER_DESC *desc, UINT *count)
{
    ...
    if (!d3d11_video_device_check_decoder(device, &desc->Guid,
            desc->OutputFormat, desc->SampleWidth, desc->SampleHeight))
    {
        *count = 0;
        return S_OK;
    }
        ...
        const D3D11_VIDEO_DECODER_DESC *desc, UINT index, D3D11_VIDEO_DECODER_CONFIG *config)
{
    ...
    config->guidConfigBitstreamEncryption = no_encrypt;
        ...
        const DXGI_RATIONAL *framerate, UINT bitrate, const GUID *encryption_type, UINT *caps)
{
    ...
    TRACE("iface %p, profile %s, width %u, height %u, framerate %u/%u, bitrate %u, encryption_type %s, caps %p.\n",
    ...
    if (encryption_type && !IsEqualGUID(encryption_type, &null_guid))
    {
        WARN("Unsupported encryption type %s.\n", debugstr_guid(encryption_type));
        *caps = D3D11_VIDEO_DECODER_CAPS_UNSUPPORTED;
        return S_OK;
    }

    if (!d3d11_video_device_check_decoder(device, profile, DXGI_FORMAT_NV12, width, height))
        *caps = D3D11_VIDEO_DECODER_CAPS_UNSUPPORTED;
    else
        *caps = 0;
