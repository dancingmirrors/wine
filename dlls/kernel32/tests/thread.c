...
static void test_NtSetLdtEntries(void)
{
    ...
    ok(!ret, "NtSetLdtEntries failed: %08lx\n", ret);
    ...
    ok(!ret, "NtQueryInformationThread failed: %08lx\n", ret);
    ds_entry = tdi.Entry;

    tdi.Selector = 0x000f;
    ret = pNtQueryInformationThread(GetCurrentThread(), ThreadDescriptorTableEntry, &tdi, sizeof(tdi), &ret);
    ok(ret == STATUS_ACCESS_VIOLATION, "got %08lx\n", ret);

    tdi.Selector = 0x001f;
    ret = pNtQueryInformationThread(GetCurrentThread(), ThreadDescriptorTableEntry, &tdi, sizeof(tdi), &ret);
    ok(ret == STATUS_ACCESS_VIOLATION, "NtQueryInformationThread returned %08lx\n", ret);
    ...
    ok(!ret || broken(ret == STATUS_INVALID_LDT_DESCRIPTOR) /*XP*/, "NtSetLdtEntries failed: %08lx\n", ret);
