...
static void test_WaitForSingleObject(void)
{
    ...
    then = GetTickCount();
    ok(ret == WAIT_TIMEOUT, "got %lu\n", ret);
    ok(abs((int)(then - now) - 100) < 5, "got %lu ms\n", then - now);

    now = GetTickCount();
    ret = WaitForSingleObject( signaled, 100 );
    then = GetTickCount();
    ok(ret == 0, "got %lu\n", ret);
    ok(abs((int)(then - now)) < 5, "got %lu ms\n", then - now);
    ...
    ok(llabs(((ntthen.QuadPart - ntnow.QuadPart) / 10000) - 100) < 5, "got %s ns\n",
    ...
    pNtQuerySystemTime( &ntthen );
    ok(status == STATUS_TIMEOUT, "got %#lx\n", status);
    ok(llabs((ntthen.QuadPart - ntnow.QuadPart) / 10000) < 5, "got %s ns\n",
        wine_dbgstr_longlong((ntthen.QuadPart - ntnow.QuadPart) * 100));
