...
/* map VK_KHR_win32_surface to / from the driver host surface extension */
static void map_instance_extensions( struct vulkan_instance_extensions *extensions )
{
    const char *host_extension = driver_funcs->p_get_host_surface_extension();

#define USE_VK_EXT(x)                                                                            \
    if (!strcmp( host_extension, #x ))                                                           \
    {                                                                                            \
        if (extensions->has_VK_KHR_win32_surface) extensions->has_ ## x = 1;                     \
        if (extensions->has_ ## x) extensions->has_VK_KHR_win32_surface = 1;                     \
    }
    ALL_VK_INSTANCE_EXTS
#undef USE_VK_EXT
}

/* map the win32 external memory / semaphore / fence extensions to their host fd counterparts */
...
static VkExternalMemoryHandleTypeFlagBits get_host_external_memory_type(void)
{
    ...
    map_device_extensions( &extensions );
...
/* WINE_VK_DISABLE_EXTENSIONS: comma separated list of host Vulkan extensions to hide, for debugging */
static BOOL is_extension_disabled( const char *name )
{
    const char *list = getenv( "WINE_VK_DISABLE_EXTENSIONS" ), *p;
    size_t len = strlen( name );

    if (!list) return FALSE;
    for (p = list; *p; p = p + strcspn( p, ",; " ), p += strspn( p, ",; " ))
    {
        size_t n = strcspn( p, ",; " );
        if (n == len && !strncmp( p, name, len )) return TRUE;
    }
    return FALSE;
}
...
static VkResult convert_instance_create_info( struct mempool *pool, VkInstanceCreateInfo *info, struct instance *instance )
{
    ...
    map_instance_extensions( &instance->obj.extensions );
                                      ...
                                      VkPhysicalDevice client_physical_device, struct vulkan_instance *instance )
{
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        if (is_extension_disabled( extension ))
        {
            WARN( "Hiding extension %s.\n", debugstr_a(extension) );
            continue;
        }
#define USE_VK_EXT(x)                           \
                                            ...
                                            struct mempool *pool, struct vulkan_device *device )
{
    ...
    map_device_extensions( &device->extensions );
                                         ...
                                         const VkAllocationCallbacks *allocator, VkDeviceMemory *ret )
{
    ...
    if (physical_device->external_memory_align && (mem_flags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) && !pointer_info &&
        ...
        return res;
    /* remember the 32-bit host allocation so that mapping and freeing use it */
...
static BOOL get_surface_rect( HWND hwnd, RECT *rect )
{
    return NtUserGetClientRect( hwnd, rect, NtUserGetDpiForWindow( hwnd ) );
}
                                         ...
                                         VkSurfaceCapabilitiesKHR *capabilities )
{
    ...
    get_surface_rect( surface->hwnd, &client_rect );
                                               ...
                                               uint32_t *image_index )
{
    ...
    if (!res && get_surface_rect( surface->hwnd, &client_rect ) &&
...
#undef USE_VK_EXT
    ...
    map_instance_extensions( &extensions );
