...
static void call_req_handler( struct thread *thread )
{
    ...
    /* Watchdog: Opt in with e.g. WINE_HOTLOOP_KILL=2000000. */
    {
        static int threshold = -1;

        if (threshold == -1)
        {
            const char *env = getenv( "WINE_HOTLOOP_KILL" );
            threshold = env ? atoi( env ) : 0;
        }
        if (threshold > 0)
        {
            static struct { thread_id_t tid; unsigned int req; unsigned int count; } loops[16];
            unsigned int i, lru = 0;

            for (i = 0; i < ARRAY_SIZE(loops); i++)
            {
                if (loops[i].tid == thread->id) break;
                if (loops[i].count < loops[lru].count) lru = i;
            }
            if (i == ARRAY_SIZE(loops))
            {
                i = lru;
                loops[i].tid = thread->id;
                loops[i].req = req;
                loops[i].count = 0;
            }

            if (loops[i].req != req) { loops[i].req = req; loops[i].count = 0; }
            else if ((int)++loops[i].count >= threshold)
            {
                fprintf( stderr, "wineserver: killing runaway pid %04x tid %04x (req %u looped %u times)\n",
                         thread->process->id, thread->id, req, loops[i].count );
                loops[i].tid = 0; loops[i].req = ~0u; loops[i].count = 0;
                kill_process( thread->process, 1 );
                current = NULL;
                return;
            }
        }
