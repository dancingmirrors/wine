...
{
    struct object          obj;             /* object header */
    struct object         *inproc_sync;     /* inproc sync for client-side waits (NULL unless ntsync) */
...
static struct msg_queue *create_msg_queue( struct thread *thread, struct thread_input *input )
{
    ...
    if ((queue = alloc_object( &msg_queue_ops )))
    {
        ...
        if (get_inproc_device_fd() >= 0 &&
            !(queue->inproc_sync = (struct object *)create_inproc_internal_sync( 1, 0 ))) goto error;
...
static void update_queue_inproc_sync( struct msg_queue *queue )
{
    if (!queue->inproc_sync) return;
    if (is_signaled( queue ) || (queue->fd && check_fd_events( queue->fd, POLLIN )))
        signal_sync( queue->inproc_sync );
    else
    {
        if (queue->fd) set_fd_events( queue->fd, POLLIN );
        reset_sync( queue->inproc_sync );
    }
}
...
static inline void set_queue_bits( struct msg_queue *queue, unsigned int bits )
{
    ...
    update_queue_inproc_sync( queue );
}
...
{
    struct msg_queue *queue = (struct msg_queue *)obj;
    if (queue->inproc_sync) return grab_object( queue->inproc_sync );
    return grab_object( &queue->obj );
}
